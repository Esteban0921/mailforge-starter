...
| TASK-0015 | Verificar docker compose (Postgres/Redis/Mailpit) en máquina real        | Agente      | Hecha     | 2026-08-26 | Puertos remapeados +1 (5433/6380/1026/8026): la máquina ya corría otro stack "crm" en los estándar                |
| TASK-0016 | Prisma: modelos User/Organization/OrganizationMember + primera migración | Agente      | Hecha     | 2026-08-26 | 3 migraciones aplicadas contra Postgres real; esquema verificado por psql (constraints, FKs, índices)             |
| TASK-0017 | Conectar PrismaModule a la API + health check de BD                      | Agente      | Hecha     | 2026-08-26 | PrismaService global (conexión perezosa); /health hace SELECT 1 y reporta database:ok/error sin tumbar el request |
| TASK-0018 | Módulo auth: registro + login + JWT access/refresh                       | Agente      | Hecha     | 2026-08-26 | bcrypt + JWT stateless (access 15m/refresh 7d, sin revocación); verificado en vivo contra Postgres real           |
| TASK-0019 | CRUD Organizations + roles + guard organizationId                        | Agente      | Hecha     | 2026-08-26 | JwtAuthGuard + OrganizationGuard (404 a no-miembros); register() ya provisiona org personal; verificado en vivo   |
| TASK-0020 | UI login/registro + layout autenticado con sidebar                       | Agente      | Hecha     | 2026-08-26 | Swap a HTTP real (client.ts elige store); E2E sigue en mock vía fixtures.ts; verificado en vivo contra Postgres   |
| TASK-0021 | Organization switcher + protección de rutas Next                         | Agente      | Hecha     | 2026-08-26 | Dropdown en el sidebar + authenticated-fetch.ts (refresh-on-401); sin mock a propósito, verificado en vivo        |
| TASK-0022 | Modelos Audience/Subscriber + endpoints CRUD + validación                | Agente      | Hecha     | 2026-08-26 | CRUD anidado en organizations/:id/audiences; Subscriber único por org (no por audiencia); verificado en vivo      |
| TASK-0023 | Importación de suscriptores por CSV                                      | Agente      | Hecha     | 2026-08-26 | POST .../subscribers/import reutiliza parseSubscriberCsv; skip por fila con motivo; verificado en vivo            |
...
| TASK-0063 | Swap de audiencias/suscriptores del frontend a la API real (TASK-0022)   | Esteban     | Pendiente | —          | El backend ya existe y está verificado; lib/audiences/store.ts (TASK-0024) sigue en mock — nadie lo ha pedido aún |
...
| TASK-0062 | Revocación de refresh tokens (logout real del lado servidor, rotación) | Esteban     | Pendiente | —          | TASK-0018 es JWT stateless a propósito: logout solo borra el token en el cliente; uno robado sigue válido hasta caducar (7d) |
...
ledger conforme se planifiquen, con ids a partir de TASK-0064, siguiendo el protocolo
